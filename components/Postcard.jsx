...
class Postcard extends React.Component{
      ...
      console.log(this.state.result);
        return (
          <div>
           ...
           id="post"
            style= {{ width: 600 }}
            actions={[<Link to= {{ pathname : '/postpage', state : {newData : this.state.result}}} ><Icon type="message" /></Link>, <Icon type="like-o" primary={true} disabled={this.state.disabledUpvote} onClick={this.handleUpvoteClicked.bind(this)}/> ,<h5>{this.state.resultA.UpVoteNumber}</h5>, <Icon type="dislike-o" secondary={true} disabled={this.state.disabledDownvote} onClick={this.handleDownvoteClicked.bind(this)}/>,<h5>{this.state.resultA.DownVoteNumber}</h5>]}>
            ...
            <CardLink style={{float:'left', fontSize:'16px'}} href="#">{this.props.some.user.UserName} </CardLink>
            <Link  to ={{pathname:'/topicpage'}} style={{float:'left' ,fontSize:'16px'}}></Link>
    ...
    export const Post = Form.create()(Postcard);